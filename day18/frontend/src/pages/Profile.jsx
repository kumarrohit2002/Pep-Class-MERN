...
const Profile = () => {
    ...
    const getData = async () => {
        try {
            const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/products`, { method: 'GET' });
            if (!response.ok) {
    ...
    return (
        <div className="min-h-screen bg-gray-100 pb-8">
            <Header />
            <h1 className="text-4xl font-extrabold text-center text-gray-800 my-8">Manage Products</h1>

            {/* --- Product Form --- */}
            <div className="max-w-xl mx-auto bg-white p-8 rounded-lg shadow-lg mb-12">
                <h2 className="text-2xl font-bold text-gray-700 mb-2 text-center">Add New Product</h2>
                <form onSubmit={handleSubmit} className="flex flex-col gap-2">
                    {/* Title */}
                    <div className="flex flex-col">
                        <label htmlFor="title" className="text-gray-700 font-semibold mb-1">Title:</label>
                        <input
                            id="title"
                            name="title"
            ...
            <div className="max-w-5xl mx-auto px-4">
                <h2 className="text-2xl font-bold text-gray-700 mb-6 text-center">Your Products</h2>
                {products.length === 0 ? (
                    <p className="text-center text-gray-500 text-lg">No products found. Add some above!</p>
                ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {products.map((pro) => (
                            <div key={pro._id} className="bg-white p-6 rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300">
                                <h3 className="text-xl font-bold text-gray-800 mb-2">{pro.title.toUpperCase()}</h3>
                                <p className="text-lg text-green-600 font-semibold mb-1">₹{pro.price}</p>
                                <p className="text-gray-600 text-sm mb-2">{pro.description}</p>
                                <p className="text-gray-500 text-sm">Quantity: {pro.quantity}</p>
                            </div>
                        ))}
                    </div>
                )}
